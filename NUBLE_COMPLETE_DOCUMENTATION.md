...
NUBLE is an **institutional-grade ML trading system** implementing Marcos López de Prado's "Advances in Financial Machine Learning" (AFML) methodology. The system has been rigorously validated and achieved production-ready status.
...
**File:** `src/nuble/news/sentiment.py` (440 lines)
...
```python
from nuble.news.client import StockNewsClient
...
**File:** `src/nuble/news/pipeline.py` (455 lines)
...
```python
from nuble.news.pipeline import NewsPipeline
...
**File:** `src/nuble/news/crypto_client.py`
...
**File:** `src/nuble/news/coindesk_client.py` (878 lines)
...
```python
from nuble.news.coindesk_client import CoinDeskClient
...
**File:** `src/nuble/assets/detector.py` (262 lines)
...
**File:** `src/nuble/router.py` (359 lines)
...
**File:** `src/nuble/services.py` (950 lines)
...
class UnifiedServices:
    """
    Unified services layer integrating all NUBLE capabilities.
...
from nuble.news.sentiment import SentimentAnalyzer
from nuble.news.coindesk_client import CoinDeskClient
from nuble.assets.detector import AssetDetector
...
Repository: https://github.com/Hlobo-dev/NUBLE-CLI
