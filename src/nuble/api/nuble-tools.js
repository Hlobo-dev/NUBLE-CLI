...
async function processNubleTool(toolName, toolContent) {
  ...
  try {
    ...
    console.error(`[NUBLE] Tool ${toolName} failed:`, err.message);
    return { 
      error: err.message, 
      tool: toolName,
      hint: `Make sure the NUBLE ROKET API is running at ${ROKET_BASE_URL}. Start it with: cd NUBLE-CLI && python -m uvicorn nuble.api.roket:app --port 8000`
    };
  }
}
