...
class MemoryManager:
    ...
    Example:
        memory = MemoryManager("~/.nuble/memory.db")
    ...
    def __init__(self, db_path: str = None):
        ...
        Args:
            db_path: Path to SQLite database file.
                     Defaults to ~/.nuble/memory.db
        """
        if db_path is None:
            home = os.path.expanduser("~")
            nuble_dir = os.path.join(home, ".nuble")
            os.makedirs(nuble_dir, exist_ok=True)
            db_path = os.path.join(nuble_dir, "memory.db")
